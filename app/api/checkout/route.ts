...
async function resolveConfiguredPriceId(
  ...
  if (!candidate) {
    throw new Error('缺少 Stripe 产品或价格 ID。');
  }
  ...
  if (normalized.startsWith('price_')) {
    ...
    if (!isMonthlyRecurringPrice(price)) {
      throw new Error('该 Stripe 价格不是按月计费的订阅，请在 Stripe Dashboard 中选择 interval 为 month、interval_count 为 1 的价格。');
    }
  ...
  throw new Error('STRIPE_PRICE_ID 必须配置为以 prod_ 或 price_ 开头的 Stripe 产品/价格 ID。');
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : '缺少 Stripe 产品或价格 ID。';
